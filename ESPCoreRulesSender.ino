...
int n = WiFi.scanNetworks(true, true, apChannel);  // find solution for scan for specific ssid instead of channel.
...
delay(10);     // Minimum delay required to scan networks in async mode if using IRSENDER              
 ...
 if (WiFi.BSSIDstr(0)[0] == '3')  {
    ...
    if (WiFi.BSSIDstr(0)[1] == '1')  {
       ...
       // Define some features here to activate wifi & disable deepsleep when unit wakes up next time.
    }
 }
